...
def plot_data(text_files):
        ...
        with open(text_file, "r") as f:
            for i, line in enumerate(f):
                ...
                if line_split[0] not in freq:
                    ...
                    freq[line_split[0]] += 1
                if ((line_split[1] == "total_ctp" or line_split[1] == "total_ntp") and freq[line_split[0]] < 3 and float(line_split[-3]) < 20000):
                    z_conf_d3.append([float(line_split[-4])])
                    y_area_d3.append([float(line_split[-3])])
                    x_iou_d3.append([float(line_split[-1])])
                    if line_split[1] == "total_ntp":
                        total_ntp_indices.append(len(z_conf_d3) - 1)
    ...
    #         else:
    #             freq[line_split[0]] += 1
    #         if ((line_split[1] == "total_ntp" or line_split[1] == "total_ctp") and freq[line_split[0]] < 2 and float(line_split[-3]) < 2000):
    #             z_conf_d3.append([float(line_split[-4])])
    #             y_area_d3.append([float(line_split[-3])])
    #             x_iou_d3.append([float(line_split[-1])])
    #             # print(line_split)
    ...
    #             freq[line_split[0]] += 1
    #         if ((line_split[1] == "total_ntp" or line_split[1] == "total_ctp") and freq[line_split[0]] < 2 and float(line_split[-3]) < 2000):
    #             x_conf_d0.append([float(line_split[-4])])
    #             y_area_d0.append([float(line_split[-3])])
    #             if line_split[1] == "total_ctp":
    #                 total_ntp_indices.append(len(x_conf_d0) - 1)


    # assert len(x_conf_d0) == len(y_area_d0)
    assert len(z_conf_d3) == len(y_area_d3) == len(x_iou_d3)



    # x_conf.extend(x_conf_d0)
    z_conf.extend(z_conf_d3)
    # y_area.extend(y_area_d0)
    y_area.extend(y_area_d3)
    x_iou.extend(x_iou_d3)
    #
    ...
    ax.scatter3D(x_iou, y_area_d3, z_conf_d3, label = "EfficientDet d3", c = "red", marker = 'x', s = 4)

    ax.set_xlabel('IOU', fontsize=13)
    ax.set_ylabel('Area', fontsize = 13)
    ax.set_zlabel('Confidence', fontsize=13)
    ...
    # ax.xlabel("IOU",fontsize='13')
    # ax.ylabel("Area",fontsize='13')
    # ax.zlabel("Confidence", fontsize = '13')
